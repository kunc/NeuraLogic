...
//        if (start != null)
        graphviz.addln(getNodeId(start) + "[label=" + GraphViz.sanitize(start.toString()) + ", color=green]");
    }

    private void drawTerminal(Object terminal) {
        if (terminal != null)
            graphviz.addln(getNodeId(terminal) + "[label=" + GraphViz.sanitize(terminal.toString()) + ", color=red]");
    }
    ...
    private void draw(Pipe pipe) {
        ...
        if (pipe instanceof IdentityGenPipe) {
            ...
            graphviz.addln(getNodeId(pipe) + "[label=" + GraphViz.sanitize(pipe.toString()) + ", shape=cds]");
        }
        if (pipe.input != null)
            drawEdge(pipe.input, pipe, "label=" + GraphViz.sanitize(types.get(0)));
        if (pipe.output != null)
            drawEdge(pipe, pipe.output, "label=" + GraphViz.sanitize(types.get(1)));
    }

    private void draw(Branch branch) {
        graphviz.addln(getNodeId(branch) + "[label=" + GraphViz.sanitize(branch.toString()) + ", shape=trapezium]");
    ...
    private void draw(Merge merge) {
        graphviz.addln(getNodeId(merge) + "[label=" + GraphViz.sanitize(merge.toString()) + ", shape=invtrapezium]");
...
//        graphviz.addln(getNodeId(pipeline) + " [shape=point, style=invis]");
        ...
        graphviz.addln("label=" + GraphViz.sanitize(pipeline.toString()) + "}");
    ...
    private static class GenericTypeGetter {
        ...
        private static String getCompoundType(Type actualTypeArgument) {
            ...
            try {
                ...
                // Commenting this out as I am was not able to solve the import error and based on
                // https://panlw.github.io/15358753472382.html , it should be possible to replace it with
                // ParameterizedType
